...
function loadDegrees(parent='.') {
	for (const file of fsutil.findFiles(parent, 'degrees')) {
		...
		for (const item of degrees) {
			validate.start();
			validate.keys(context, item, ['degree', 'format'], []);
...
function loadDimensions(parent='.') {
	for (const file of fsutil.findFiles(parent, 'dimensions')) {
		...
		for (const id of Object.keys(dimensions)) {
			...
			validate.start();
			validate.keys(context, item, ['legacy-id', 'name'], ['symbol', 'composable']);
...
function loadUnitTypes(parent='.', lang='en') {
	for (const file of fsutil.findFiles(parent, 'unit-types')) {
		...
		for (const id of Object.keys(unitTypes)) {
			...
			validate.keys(context, item, ['name'], ['icon', 'dimension']);
	...
	for (const file of fsutil.findFiles(parent, 'units')) {
		...
		for (const id of Object.keys(units)) {
			...
			if (item['datatype'] === undefined || item['datatype'] === 'num') {
				validate.keys(context, item, ['name'], ['symbol', 'datatype', 'multiplier', 'divisor', 'instructions', 'parser', 'formatter', 'dimension']);
			} else if (item['datatype'] === 'text') {
				validate.keys(context, item, ['name', 'datatype', 'parser', 'formatter'], ['symbol', 'dimension']);
			} else if (item['datatype'] === 'tuple') {
				validate.keys(context, item, ['name', 'datatype', 'tuple-dimension', 'parser', 'formatter'], ['symbol', 'dimension']);
			} else if (item['datatype'] === 'dep') {
				validate.keys(context, item, ['name', 'datatype', 'dep-name', 'parser', 'formatter'], ['symbol', 'dep-dimension', 'dimension']);
			} else if (item['datatype'] === 'cc') {
				validate.keys(context, item, ['name', 'datatype', 'cc-map', 'parser', 'formatter'], ['symbol', 'dimension']);
			} else {
...
function loadSolvers(parent='.', lang='en') {
	for (const file of fsutil.findFiles(parent, 'solvers')) {
		...
		for (const id of Object.keys(solvers)) {
			...
			validate.keys(context, item, ['name', 'variables', 'solutions'], []);
			validate.ls(context, 'name', item['name']);
