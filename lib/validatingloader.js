			...
			if (item['categories'] && typeof item['categories'] === 'object' && item['categories'].length) {
				for (const cat of item['categories']) {
					if (cat['include'] !== undefined) {
						...
						if (!includeMap[includeId] && ['i36','i162'].indexOf(includeId) < 0) {
							validate.error(context, 'value for "include" references undefined include "' + includeId + '"');
						}
					} else {
						validate.keys(context, cat, ['type', 'units']);
						const unitTypeId = (cat['type'] = String(cat['type']).replace(/\s*#.*$/g, ''));
						const catType = unitTypeMap[unitTypeId];
						if (!catType) {
							validate.error(context, 'value for "type" references undefined unit type "' + unitTypeId + '"');
						}
...
function listRemainingAmbiguities(logger) {
	if (logger === undefined) { logger = validate.logger(); } if (!logger) return;
	remainingAmbiguities.sort().map(a => logger.log(a));
}
...
module.exports = {
	...
	loadDisambiguation, load, summarize, listRemainingAmbiguities
};
