...
function srmpValidate(s) {
	s = s.replaceAll(srmpPattern, '');
	return !s.replaceAll(/\s/g, '');
}

function srmpParseForward(s, opcode='M') {
	const operations = [];
...
function srmpParseReverse(s, opcode='M') {
	const reverse = [];
	const forward = srmpParseForward(s, opcode);
	while (forward.length) {
		const [opcode, operand] = forward.pop();
		reverse.push([(srmpReverseOp[opcode] || opcode), operand]);
	}
	return reverse;
}
...
function srmpCodeGenForward(s, cgop=srmpCodeGenOperation) {
	let kernel = 'a';
	for (const [opcode, operand] of srmpParseForward(s, 'M')) {
...
function srmpCodeGenReverse(s, cgop=srmpCodeGenOperation) {
	let kernel = 'a';
	for (const [opcode, operand] of srmpParseReverse(s, 'M')) {
		kernel = cgop(kernel, opcode, operand);
	}
	return 'function(a){return(' + kernel + ');}';
}
...
module.exports = {
	validate: srmpValidate,
	parseForward: srmpParseForward,
	parseReverse: srmpParseReverse,
