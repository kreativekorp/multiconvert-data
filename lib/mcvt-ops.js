...
function twrt(a) {
	if (type(a) === 'value') return Math.sqrt(Math.sqrt(Math.cbrt(a)));
	return pow(a, 1/12);
}
...
function acot(a) {
	if (type(a) === 'value') return Math.atan2(1, a);
...
function eml(a, b) {
	return sub(exp(a), ln(b));
}

function edl(a, b) {
	return div(exp(a), ln(b));
}

function lme(a, b) {
	return sub(ln(a), exp(b));
}

function lde(a, b) {
	return div(ln(a), exp(b));
}
...
function erfcx(x) {
	...
	if (x == 0) return 1;
	if (!isFinite(x)) return 0;
	let y = 0.56418958354775629 / (x + 2.06955023132914151);
...
function erfi(x) {
	...
	let [a,b] = cdiv(0.56418958354775629, 0, 2.06955023132914151, x);
	[a,b] = cmul(a, b, ...cdiv(5.80755613130301624 - x*x, 2.71078540045147805*x, 12.06166887286239555 - x*x, 3.47954057099518960*x));
	[a,b] = cmul(a, b, ...cdiv(12.07402036406381411 - x*x, 3.47469513777439592*x, 8.44319781003968454 - x*x, 3.72068443960225092*x));
	[a,b] = cmul(a, b, ...cdiv(9.30596659485887898 - x*x, 4.00561509202259545*x, 6.36161630953880464 - x*x, 3.90225704029924078*x));
	[a,b] = cmul(a, b, ...cdiv(9.12661617673673262 - x*x, 5.16722705817812584*x, 5.13578530585681539 - x*x, 4.03296893109262491*x));
	[a,b] = cmul(a, b, ...cdiv(9.19435612886969243 - x*x, 5.95908795446633271*x, 4.48640329523408675 - x*x, 4.11240942957450885*x));
	if (isNaN(b)) return (x < 1) ? 0 : Infinity;
	return -b * Math.exp(x*x);
}

function __gamma_tzx(z) {
	const g = 7;
	const p = [
...
function gamma(z) {
	...
	const [t, z2, x] = __gamma_tzx(z);
	return Math.sqrt(Math.PI * 2) * Math.pow(t, z2) * Math.exp(-t) * x;
}
...
function lgamma(z) {
	...
	if (z == 1 || z == 2) return 0;
	if (!isFinite(z)) return z;
	if (z < 0.5) return Math.log(Math.abs(Math.PI / Math.sin(Math.PI * z))) - lgamma(1 - z);
	const [t, z2, x] = __gamma_tzx(z);
	return Math.log(Math.sqrt(Math.PI * 2)) + z2 * Math.log(t) - t + Math.log(x);
}
...
function lnPr(n, r) {
	return lgamma(n + 1) - lgamma(n - r + 1);
}
...
function min(...a) {
	return (a.length === 0) ? NaN : a.reduce((a,b) => (cmp(a,b)>0)?b:a);
}
...
module.exports = {
	...
	abs, sign, sqrt, cbrt, qtrt, twrt, random, randomDecimal, randomRange,
	hypot, atan2, xcoord, ycoord, annuity, compound,
	fma, eml, edl, lme, lde, agm, gcd, lcm, erf, erfc, erfcx, erfi,
	gamma, lgamma, beta, lbeta, nCr, lnCr, nPr, lnPr,
	reverseBits, reverseBytes, bitLength, bitMingle, bitSelect,
	sum, altsum, prod, altprod, min, max,
	rsr, rms, average, geomean, harmean,
