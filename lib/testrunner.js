...
function mcparse(u, a, depInputs) {
	const p = unit.parserFor(u);
	if (depInputs && u['dep-name'] && depInputs[ls.get(u['dep-name'])] !== undefined) {
		return p(a, depInputs[ls.get(u['dep-name'])]);
	} else {
		return p(a);
	}
}
...
function mcformat(u, a, depInputs) {
	const f = unit.formatterFor(u);
	if (depInputs && u['dep-name'] && depInputs[ls.get(u['dep-name'])] !== undefined) {
		return f(a, depInputs[ls.get(u['dep-name'])]);
	} else {
		return f(a);
	}
}
				...
				if (value && typeof value === 'object' && value.length) {
					for (let i = 0; i < value.length; i++) {
						if (value[i] && typeof value[i] === 'object') {
							if ('replace' in value[i] && 'with' in value[i]) {
								const s = new RegExp(String(value[i]['replace']), 'gu');
								const r = String(value[i]['with']);
								replacements.push([s, r]);
							} else {
								validate.error(context, 'value for "replacements" at index ' + i + ' must have keys "replace" and "with"');
							}
						} else {
							validate.error(context, 'value for "replacements" at index ' + i + ' must be a non-null object');
						}
					}
				} else {
					validate.error(context, 'value for "replacements" must be a non-empty array');
				}
			} else if (key === 'epsilon') {
				epsilon = Math.abs(value);
			} else if (key === 'inputs') {
			...
			for (const [iu, iv] of inputs) {
				...
				for (const [ou, ov] of outputs) {
					...
					if (!iu || !ou || da.eq(iu['dimension'], ou['dimension'])) {
						let v = ou ? mcformat(ou, baseValue, depInputs) : baseValue;
						for (const [s, r] of replacements) {
							v = String(v).replaceAll(s, r);
						}
						...
						let v = mcformat(ou, 1 / baseValue, depInputs);
						for (const [s, r] of replacements) {
							v = String(v).replaceAll(s, r);
						}
						if (testEqual(v, ov, epsilon)) {
