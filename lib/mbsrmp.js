...
class MBSRMPKernel {
	...
	indexOfRegister(r) {
		const key = String(r);
		const index = this.constants.indexOf(key);
		if (index >= 0) return index;
		const newIndex = this.constants.length;
	...
	compile() {
		...
		const size = kptr + this.constants.length * 5;
	...
	dump(logger=console) {
		logger.log('Forward:');
		for (const fi of this.forwardInstructions) {
			const h = (fi | 0x100).toString(16).substring(1).toUpperCase();
			if (fi >= LDARG) {
				logger.log(h, 'LDARG', fi & 0x7F);
			} else if (fi >= STFAC) {
				logger.log(h, 'STFAC', fi & 0x3F);
			} else {
				logger.log(h, OPCODES[fi & 0x1F]);
			}
		}
		logger.log('Reverse:');
		for (const ri of this.reverseInstructions) {
			const h = (ri | 0x100).toString(16).substring(1).toUpperCase();
			if (ri >= LDARG) {
				logger.log(h, 'LDARG', ri & 0x7F);
			} else if (ri >= STFAC) {
				logger.log(h, 'STFAC', ri & 0x3F);
			} else {
				logger.log(h, OPCODES[ri & 0x1F]);
			}
		}
		logger.log('Constants:');
		for (const k of this.constants) {
			const h = (k | (1n << 40n)).toString(16).substring(1).toUpperCase();
			logger.log(h, mbf.decode(k));
		}
	}
}

function mbsrmpCompileInstructions(s, complement=false) {
	const kernel = new MBSRMPKernel();
	kernel.parseInstructions(s, complement);
	return kernel.compile();
}

function mbsrmpCompileRational(p, q, complement=false) {
	const kernel = new MBSRMPKernel();
	kernel.parseRational(p, q, complement);
	return kernel.compile();
}

function mbsrmpCompileBase(complement=false) {
	const kernel = new MBSRMPKernel();
	kernel.parseBase(complement);
	return kernel.compile();
}

function mbsrmpDump(view, logger=console) {
	const kernel = new MBSRMPKernel();
...
function mbsrmpDumpCategory(view, namefn=defaultNameDecodeFn, logger=console) {
	...
	for (const [name, pptr, plen] of index) {
		...
		const slice = view.buffer.slice(start, end);
		mbsrmpDump(new DataView(slice), logger);
	}
}
...
function mbsrmpDumpInclude(view, namefn=defaultNameDecodeFn, logger=console) {
	const length = view.getUint8(1, true);
	const sotptr = view.getUint16(2, true);
	const potptr = view.getUint16(4, true);
	const index = [];
	...
	for (const [name, pptr, plen] of index) {
		...
		const slice = view.buffer.slice(start, end);
		mbsrmpDumpCategory(new DataView(slice), namefn, logger);
	}
}
